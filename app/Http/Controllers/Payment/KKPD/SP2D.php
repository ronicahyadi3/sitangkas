...
class SP2D extends Controller
{
    ...
    {
        Log::channel(self::LOG_CHANNEL)->debug('SP2D KKPD index accessed');

        return view('Payment.KKPD.sp2d');
    }
    ...
    public function json(ActivePositionService $activePosition)
    {
        ...
        try {
            ...
            if (! $user || ! $user->jabatan || ! $user->unitKerja) {
                Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD json blocked: invalid active position', [
            ...
            if (! in_array($jabatanId, [2, 3, 4, 13], true)) {
                Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD json blocked: forbidden role', [
                    'duration_ms' => $this->durationMs($start),
                ]);
            ...
            Log::channel(self::LOG_CHANNEL)->debug('SP2D KKPD json request');

            $query = KKPD::rootQueryAlias('sp2d')
                    ...
                    if (in_array($jabatanId, [2, 3], true)) {
                        if (is_null($row->rejected_by) && is_null($row->finished_at)) {
                            $actions[] = $btn(
                                ...
                                'Menolak Data',
                                'data-payment="KKPD" data-type="SP2D"'
                            );
                        }
                        $actions[] = $btn($encRef, 'history_data', 'ni ni-collection text-info', 'History');
            ...
            Log::channel(self::LOG_CHANNEL)->debug('SP2D KKPD json success', [
                'duration_ms' => $this->durationMs($start),
            ]);

            return $response;
        } catch (\Throwable $e) {
            Log::channel(self::LOG_CHANNEL)->error('SP2D KKPD json failed', [
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
            report($e);
    ...
    public function store(
        ...
        if (! $this->canCrud($user)) {
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD store blocked: forbidden role', [
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        Log::channel(self::LOG_CHANNEL)->info('SP2D KKPD store request', [
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD store invalid selected_spm', [
                'selected_spm' => $validated['selected_spm'],
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->info('SP2D KKPD store blocked', [
                'selected_spm' => $validated['selected_spm'],
                'reason' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->info('SP2D KKPD store success', [
                'nomor_sp2d' => $validated['nomor_sp2d'],
                'reference_id' => $referenceId,
                'duration_ms' => $this->durationMs($start),
            ]);
            ...
            Log::channel(self::LOG_CHANNEL)->error('SP2D KKPD store failed', [
    ...
    {
        $start = microtime(true);
        Log::channel(self::LOG_CHANNEL)->debug('SP2D KKPD edit request', [
            'hash' => $request->id,
        ]);

        $user = $activePosition->get();
        if (! $this->canCrud($user)) {
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD edit blocked: forbidden role', [
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD edit invalid hash', [
                'hash' => $request->id,
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if (! $sp2d) {
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD edit not found', [
                'sp2d_id' => $id,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if ($this->isLockedAfterSigned($sp2d)) {
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD edit blocked: signed document', [
                'sp2d_id' => $sp2d->id,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        Log::channel(self::LOG_CHANNEL)->debug('SP2D KKPD edit success', [
            'sp2d_id' => $sp2d->id,
            'duration_ms' => $this->durationMs($start),
        ]);
    ...
    public function update(
        ...
        $user = $activePosition->get();
        if (! $this->canCrud($user)) {
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD update blocked: forbidden role', [
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD update invalid hash', [
                'hash' => $id,
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        Log::channel(self::LOG_CHANNEL)->info('SP2D KKPD update request', [
            'hash' => $id,
            'nomor_sp2d' => $validated['nomor_sp2d'],
            'selected_spm' => $validated['selected_spm'],
            'has_file' => $request->hasFile('file_sp2d'),
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD update invalid selected_spm', [
                'selected_spm' => $validated['selected_spm'],
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if (! $sp2d) {
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD update not found', [
                'sp2d_id' => $sp2dId,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if ($this->isLockedAfterSigned($sp2d)) {
            Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD update blocked: signed document', [
                'sp2d_id' => $sp2d->id,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->info('SP2D KKPD update blocked', [
                'sp2d_id' => $sp2dId,
                'selected_spm' => $validated['selected_spm'],
                'reason' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->info('SP2D KKPD update success', [
                'sp2d_id' => $sp2dId,
                'reference_id' => $referenceId,
                'duration_ms' => $this->durationMs($start),
            ]);
            ...
            $this->cleanupStoredFiles($storedFiles);
            Log::channel(self::LOG_CHANNEL)->error('SP2D KKPD update failed', [
                'sp2d_id' => $sp2dId,
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
    ...
    public function formJson(Request $request, ActivePositionService $activePosition)
    {
        ...
        try {
            $user = $activePosition->get();
            if (! $this->canCrud($user)) {
                Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD formJson blocked: forbidden role', [
                    'duration_ms' => $this->durationMs($start),
                ]);
            ...
            if ($request->data) {
                try {
                    ...
                    Log::channel(self::LOG_CHANNEL)->warning('SP2D KKPD formJson invalid hash', [
                        'hash' => $request->data,
                        'error' => $e->getMessage(),
                        'duration_ms' => $this->durationMs($start),
                    ]);
            ...
            Log::channel(self::LOG_CHANNEL)->debug('SP2D KKPD formJson request', [
                'edited' => $isEdited,
            ...
            $response = DataTables::of($query)
                ...
                ->addColumn('action', function ($row) use ($request, $referenceId) {
                    ...
                    $deniedButton = $checked ? '' : '<button type="button" class="btn btn-outline-danger denied" value="'.EncryptedId::encode($row->id).'" data-wenk="Menolak data" data-payment="KKPD" data-type="SPM" data-wenk-color="red"><i class="fa-solid fa-ban"></i></button>';
            ...
            Log::channel(self::LOG_CHANNEL)->debug('SP2D KKPD formJson success', [
                'duration_ms' => $this->durationMs($start),
            ]);

            return $response;
        } catch (\Throwable $e) {
            Log::channel(self::LOG_CHANNEL)->error('SP2D KKPD formJson failed', [
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
            report($e);
