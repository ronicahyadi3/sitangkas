...
class LPJ extends Controller
{
    ...
    public function index(ActivePositionService $activePosition)
    {
        ...
        if (! in_array($jabatanId, [5, 6, 9, 10, 13], true)) {
            abort(403, 'Anda tidak berwenang mengakses LPJ KKPD.');
        }

        return view('Payment.KKPD.lpj');
    }
    ...
    public function json(ActivePositionService $activePosition)
    {
        ...
        try {
            ...
            $query = $this->buildPackageQuery()
                ->when(true, function ($q) use ($jabatanId, $unitKerjaId) {
                    ...
                    $q->where(function ($scope) use ($unitKerjaId) {
                        ...
                        $scope->whereRaw("FIND_IN_SET('5', REPLACE(COALESCE(sts.assigned_to, ''), ' ', ''))")
                            ->orWhereRaw("FIND_IN_SET('6', REPLACE(COALESCE(sts.assigned_to, ''), ' ', ''))")
                            ->orWhereRaw("FIND_IN_SET('5', REPLACE(COALESCE(lpj.assigned_to, ''), ' ', ''))")
                            ->orWhereRaw("FIND_IN_SET('6', REPLACE(COALESCE(lpj.assigned_to, ''), ' ', ''))")
                            ->orWhereRaw("FIND_IN_SET('5', REPLACE(COALESCE(tbp.assigned_to, ''), ' ', ''))")
                            ->orWhereRaw("FIND_IN_SET('6', REPLACE(COALESCE(tbp.assigned_to, ''), ' ', ''))");
                    });
                });
                ...
                ->addColumn('action', function ($row) use ($btn, $jabatanId) {
                    ...
                    if (in_array($jabatanId, [9, 10], true) && $this->canManageCrud($row, $jabatanId)) {
                        $actions[] = $btn(
                            ...
                            'Kelola Dokumen LPJ',
                            sprintf(
                                'data-reference="%s" data-sp2d="%s" data-unit="%s" data-sp2d-number="%s" data-spp-number="%s"',
                                $referenceHash,
                                $sp2dHash,
                                e((string) $row->unit_kerja),
                                e((string) ($row->nomor_sp2d ?? '-')),
            ...
            Log::channel(self::LOG_CHANNEL)->error('LPJ KKPD json failed', [
                'error' => $e->getMessage(),
            ...
            return response()->json([
                'status' => 500,
                'message' => 'Gagal memuat data LPJ KKPD.',
            ], 500);
        }
    }
    ...
    private function buildPackageQuery()
    {
        ...
        return PaymentKKPD::rootQueryAlias('sp2d')
    ...
    private function persistChildDocument(
        ...
        if (! $user || ! $user->jabatan || ! $user->unitKerja || ! in_array((int) $user->jabatan->id, [9, 10], true)) {
            return response()->json(['status' => 403, 'message' => 'Anda tidak berwenang mengelola LPJ KKPD.'], 403);
        }
        ...
        if ($documentHash) {
            ...
            if (
                ...
                ! $this->canManageChildDocument($existingDocument, $user)
            ) {
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->error('LPJ KKPD save failed', [
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            report($e);
    ...
    private function canSubmitPackage($row, int $jabatanId): bool
    {
        ...
        if (in_array($jabatanId, [9, 10], true)) {
            return $this->initiatorHasSignedAllDocuments($row, $jabatanId)
                && $this->resolveFlowRole($row) === (string) $jabatanId
                && ! $this->initiatorHasSubmittedAllDocuments($row, $jabatanId);
        }

        if (in_array($jabatanId, [5, 6], true)) {
            return $this->approverHasSignedRequiredDocuments($row, $jabatanId)
                && $this->resolveFlowRole($row) === ($jabatanId === 5 ? '9' : '10')
    ...
    private function canDenyPackage($row, int $jabatanId): bool
    {
        ...
        return $this->packageHasDocuments($row)
            && $this->resolveFlowRole($row) === ($jabatanId === 5 ? '9' : '10');
    }
    ...
    private function canViewPackage($package, int $jabatanId, int $unitKerjaId): bool
    {
        ...
        return (int) $package->id_unit_kerja === $unitKerjaId
            || (int) ($package->skpd_id ?? 0) === $unitKerjaId;
    }
    ...
    private function resolveFlowRole($row): string
    {
        ...
        if (
            $this->csvContainsRole($row->submit_tbp ?? null, '10')
