...
class SPM extends Controller
{
    ...
    {
        Log::channel(self::LOG_CHANNEL)->debug('SPM KKPD index accessed');

        return view('Payment.KKPD.spm');
    }
    ...
    public function json(ActivePositionService $activePosition)
    {
        ...
        try {
            ...
            if (! $user || ! $user->jabatan || ! $user->unitKerja) {
                Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD json blocked: invalid active position', [
            ...
            if (! in_array($jabatanId, [4, 5, 7, 13], true)) {
                Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD json blocked: forbidden role', [
                    'duration_ms' => $this->durationMs($start),
                ]);
            ...
            Log::channel(self::LOG_CHANNEL)->debug('SPM KKPD json request');

            $query = KKPD::rootQueryAlias('spm')
            ...
            $response = DataTables::of($query)
                ...
                ->addColumn('status_button', function ($row) use ($jabatanId) {
                    ...
                    if (! is_null($row->rejected_by)) {
                        return '<span type="button" class="btn btn-sm btn-danger show-document" data-id="'.$enc.'" data-wenk="'.e((string) $row->notes).'" data-wenk-color="red" data-toggle="modal" data-target="#FormTTE"><i class="far fa-file-excel"></i> '.$this->rejectedLabel((int) $row->rejected_by).'</span>';
                    }
                    ...
                    if ($jabatanId === 4) {
                        return '<span type="button" class="btn btn-sm btn-warning show-document" data-id="'.$enc.'" data-wenk="Belum Verifikasi" data-wenk-color="orange" data-toggle="modal" data-target="#FormTTE"><i class="fa-solid fa-triangle-exclamation"></i> Belum Verifikasi</span>';
                    }
                    ...
                    if ($jabatanId === 7) {
                        ...
                        if (in_array('7', $statusSp, true)) {
                            return '<span type="button" class="btn btn-sm btn-secondary show-document" data-id="'.$enc.'" data-wenk="Belum Submit" data-wenk-color="blue" data-toggle="modal" data-target="#FormTTE"><i class="fas fa-hourglass-half"></i> Belum Submit</span>';
                        }
                ...
                ->addColumn('action', function ($row) use ($btn, $jabatanId) {
                    ...
                    if ($jabatanId === 7 && (empty($submitArr) || $isRejected)) {
                        ...
                        $actions[] = $btn($enc, 'delete', 'fas fa-trash text-danger', 'Hapus', 'data-payment="KKPD" data-type="SPM"');
                    }
                    ...
                    if (
                        ...
                        $actions[] = $btn($enc, 'denied', 'far fa-file-excel text-danger', 'Menolak Data', 'data-payment="KKPD" data-type="SPM"');
                        if ($paSignedAll) {
                            $actions[] = $btn($enc, 'submit_data', 'ni ni-send text-success', 'Submit');
                        }
                    }
                    ...
                    if (
                        ...
                        $actions[] = $btn($enc, 'verify_data', 'fas fa-user-check text-success', 'Verifikasi', 'data-payment="KKPD" data-type="SPM"');
                        $actions[] = $btn($enc, 'denied', 'far fa-file-excel text-danger', 'Menolak Data', 'data-payment="KKPD" data-type="SPM"');
                    }
            ...
            Log::channel(self::LOG_CHANNEL)->debug('SPM KKPD json success', [
                'duration_ms' => $this->durationMs($start),
            ]);

            return $response;
        } catch (\Throwable $e) {
            Log::channel(self::LOG_CHANNEL)->error('SPM KKPD json failed', [
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
            report($e);
    ...
    public function submit(
        ...
        Log::channel(self::LOG_CHANNEL)->info('SPM KKPD submit request', [
            'hash' => $request->id,
        ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD submit invalid hash', [
                'hash' => $request->id,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        $user = $activePosition->get();
        if (! $user || ! $user->jabatan || ! $user->unitKerja) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD submit blocked: invalid active position', [
                'spm_id' => $spmId ?? null,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if (! in_array((int) $user->jabatan->id, [7, 5], true)) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD submit blocked: forbidden role', [
                'spm_id' => $spmId ?? null,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->info('SPM KKPD submit success', [
                'spm_id' => $spmId,
            ...
            Log::channel(self::LOG_CHANNEL)->info('SPM KKPD submit blocked', [
                'spm_id' => $spmId ?? null,
            ...
            Log::channel(self::LOG_CHANNEL)->error('SPM KKPD submit failed', [
                'spm_id' => $spmId ?? null,
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
    ...
    public function formJson(Request $request, ActivePositionService $activePosition)
    {
        ...
        try {
            $user = $activePosition->get();
            if (! $user || ! $user->jabatan || ! $user->unitKerja) {
                Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD formJson blocked: invalid active position', [
                    'duration_ms' => $this->durationMs($start),
                ]);
            ...
            if (! $this->canCrud($user)) {
                Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD formJson blocked: forbidden role', [
                    'duration_ms' => $this->durationMs($start),
                ]);
            ...
            if ($request->data) {
                try {
                    ...
                    Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD formJson invalid hash', [
                        'hash' => $request->data,
                        'duration_ms' => $this->durationMs($start),
                    ]);
            ...
            Log::channel(self::LOG_CHANNEL)->debug('SPM KKPD formJson request', [
                'hash' => $request->data,
            ...
            $response = DataTables::of($query)
                ...
                ->addColumn('action', function ($data) use ($currentSppId) {
                    ...
                    $deniedButton = $checked ? '' : '<button type="button" class="btn btn-outline-danger denied" value="'.$id.'" data-wenk="Menolak data" data-payment="KKPD" data-type="SPP" data-wenk-color="red"><i class="fa-solid fa-ban"></i></button>';
            ...
            Log::channel(self::LOG_CHANNEL)->debug('SPM KKPD formJson success', [
                'current_spp_id' => $currentSppId,
                'duration_ms' => $this->durationMs($start),
            ]);
            ...
            Log::channel(self::LOG_CHANNEL)->error('SPM KKPD formJson failed', [
                'hash' => $request->data,
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
    ...
    public function store(Request $request, ActivePositionService $activePosition, DocumentHistoryService $documentHistoryService)
    {
        ...
        Log::channel(self::LOG_CHANNEL)->info('SPM KKPD store request', [
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD store blocked: invalid selected SPP', [
                'selected_spp' => $request->input('selected_spp'),
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        $user = $activePosition->get();
        if (! $user || ! $user->jabatan || ! $user->unitKerja) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD store blocked: invalid active position', [
        ...
        if (! $this->canCrud($user)) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD store blocked: forbidden role', [
                'selected_spp' => $sppId ?? null,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->info('SPM KKPD store success', [
                'selected_spp' => $sppId,
                'nomor_spm' => $request->input('nomor_spm'),
                'duration_ms' => $this->durationMs($start),
            ]);
            ...
            Log::channel(self::LOG_CHANNEL)->info('SPM KKPD store blocked', [
                'selected_spp' => $sppId ?? null,
                'message' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
            ...
            $this->cleanupStoredFiles($storedFiles);
            Log::channel(self::LOG_CHANNEL)->error('SPM KKPD store failed', [
                'selected_spp' => $sppId ?? null,
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ]);
    ...
    public function edit(Request $request, ActivePositionService $activePosition)
    {
        ...
        Log::channel(self::LOG_CHANNEL)->debug('SPM KKPD edit request', [
            'hash' => $request->id,
        ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD edit invalid hash', [
                'hash' => $request->id,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        $user = $activePosition->get();
        if (! $user || ! $user->jabatan || ! $user->unitKerja) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD edit blocked: invalid active position', [
                'spm_id' => $spmId ?? null,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if (! $this->canCrud($user)) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD edit blocked: forbidden role', [
                'spm_id' => $spmId ?? null,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if (! $spm) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD edit not found/forbidden', [
                'spm_id' => $spmId,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if (! $this->canEditDraftOrRejected($spm)) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD edit blocked: submitted document', [
                'spm_id' => $spm->id,
                'submit' => $spm->submit,
        ...
        Log::channel(self::LOG_CHANNEL)->debug('SPM KKPD edit success', [
            'spm_id' => $spm->id,
    ...
    public function update(Request $request, string $id, ActivePositionService $activePosition, DocumentHistoryService $documentHistoryService)
    {
        ...
        Log::channel(self::LOG_CHANNEL)->info('SPM KKPD update request', [
            'hash' => $id,
            'selected_spp' => $request->input('selected_spp'),
            'nomor_spm' => $request->input('nomor_spm'),
            'nomor_sptjm' => $request->input('nomor_sptjm'),
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD update invalid hash', [
                'hash' => $id,
                'selected_spp' => $request->input('selected_spp'),
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        $user = $activePosition->get();
        if (! $user || ! $user->jabatan || ! $user->unitKerja) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD update blocked: invalid active position', [
                'spm_id' => $spmId ?? null,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        if (! $this->canCrud($user)) {
            Log::channel(self::LOG_CHANNEL)->warning('SPM KKPD update blocked: forbidden role', [
                'spm_id' => $spmId ?? null,
                'duration_ms' => $this->durationMs($start),
            ]);
        ...
        try {
            ...
            Log::channel(self::LOG_CHANNEL)->info('SPM KKPD update success', [
                'spm_id' => $spmId,
                'selected_spp' => $sppId,
                'duration_ms' => $this->durationMs($start),
            ]);
            ...
            Log::channel(self::LOG_CHANNEL)->info('SPM KKPD update blocked', [
                'spm_id' => $spmId ?? null,
                'selected_spp' => $sppId ?? null,
                'message' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
            ...
            $this->cleanupStoredFiles($storedFiles);
            Log::channel(self::LOG_CHANNEL)->error('SPM KKPD update failed', [
                'spm_id' => $spmId ?? null,
                'selected_spp' => $sppId ?? null,
                'error' => $e->getMessage(),
                'duration_ms' => $this->durationMs($start),
