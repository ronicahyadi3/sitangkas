...
class SP2D extends Controller
{
    ...
    public function json(ActivePositionService $activePosition)
    {
        ...
        try {
            ...
            if (! $user || ! $user->jabatan || ! $user->unitKerja) {
                Log::channel('payment_tu')->warning('SP2D TU json blocked: invalid active position', [
            ...
            Log::channel('payment_tu')->debug('SP2D TU json request');

            $query = PaymentTU::rootQueryAlias('sp2d')
                    ...
                    if (in_array($jabatanId, [2, 3], true)) {
                        if (is_null($row->rejected_by)) {
                            $actions[] = $btn(
                                ...
                                'Menolak Data',
                                'data-payment="TU" data-type="SP2D"'
                            );
                        }
                        $actions[] = $btn($encRef, 'history_data', 'ni ni-collection text-info', 'History');
            ...
            Log::channel('payment_tu')->debug('SP2D TU json success', [
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);

            return $response;
        } catch (\Throwable $e) {
            Log::channel('payment_tu')->error('SP2D TU json failed', [
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            report($e);
    ...
    public function store(
        ...
        if (! $this->canCrud($user)) {
            Log::channel('payment_tu')->warning('SP2D TU store blocked: forbidden role', [
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        Log::channel('payment_tu')->info('SP2D TU store request', [
        ...
        try {
            ...
            Log::channel('payment_tu')->warning('SP2D TU store invalid selected_spm', [
                'hash' => $validated['selected_spm'],
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            ...
            Log::channel('payment_tu')->info('SP2D TU store blocked', [
                'reason' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            ...
            Log::channel('payment_tu')->info('SP2D TU store success', [
                'nomor' => $validated['nomor_sp2d'],
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_tu')->error('SP2D TU store failed', [
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            report($e);
    ...
    {
        $start = microtime(true);
        Log::channel('payment_tu')->debug('SP2D TU edit request', [
            'hash' => $request->id,
        ]);

        $user = $activePosition->get();
        if (! $this->canCrud($user)) {
            Log::channel('payment_tu')->warning('SP2D TU edit blocked: forbidden role', [
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            ...
            Log::channel('payment_tu')->warning('SP2D TU edit invalid hash', [
                'hash' => $request->id,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        if (! $sp2d) {
            Log::channel('payment_tu')->warning('SP2D TU edit not found', [
                'doc_id' => $id,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        if ($this->isLockedAfterSigned($sp2d)) {
            Log::channel('payment_tu')->warning('SP2D TU edit blocked: signed document', [
                'doc_id' => $sp2d->id,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        Log::channel('payment_tu')->debug('SP2D TU edit success', [
            'doc_id' => $sp2d->id,
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);
    ...
    public function update(
        ...
        $user = $activePosition->get();
        if (! $this->canCrud($user)) {
            Log::channel('payment_tu')->warning('SP2D TU update blocked: forbidden role', [
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            ...
            Log::channel('payment_tu')->warning('SP2D TU update invalid hash', [
                'hash' => $id,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        Log::channel('payment_tu')->info('SP2D TU update request', [
            'hash' => $id,
            'nomor' => $validated['nomor_sp2d'],
            'has_file' => $request->hasFile('file_sp2d'),
            'has_user' => filled($validated['user'] ?? null),
        ...
        try {
            ...
            Log::channel('payment_tu')->warning('SP2D TU update invalid selected_spm', [
                'hash' => $validated['selected_spm'],
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        if (! $sp2d) {
            Log::channel('payment_tu')->warning('SP2D TU update not found', [
                'doc_id' => $sp2dId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        if ($this->isLockedAfterSigned($sp2d)) {
            Log::channel('payment_tu')->warning('SP2D TU update blocked: signed document', [
                'doc_id' => $sp2d->id,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            ...
            Log::channel('payment_tu')->info('SP2D TU update blocked', [
                'doc_id' => $sp2dId,
                'reason' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            DB::transaction(function () use ($validated, $request, $user, $referenceId, $spm, $sp2d, &$storedFiles, $documentHistoryService) {
                ...
                $documentHistoryService->edited($sp2d->id, $filenameForHistory, $sp2d->id_unit_kerja);
            });

            Log::channel('payment_tu')->info('SP2D TU update success', [
                'doc_id' => $sp2dId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            $this->cleanupStoredFiles($storedFiles);
            Log::channel('payment_tu')->error('SP2D TU update failed', [
                'doc_id' => $sp2dId,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
    ...
    public function formJson(Request $request, ActivePositionService $activePosition)
    {
        ...
        $user = $activePosition->get();
        if (! $this->canCrud($user)) {
            Log::channel('payment_tu')->warning('SP2D TU formJson blocked: forbidden role', [
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        Log::channel('payment_tu')->debug('SP2D TU formJson request', [
            'edited' => $isEdited,
        ...
        if ($request->data) {
            try {
                ...
                Log::channel('payment_tu')->warning('SP2D TU formJson invalid hash', [
                    'hash' => $request->data,
                    'error' => $e->getMessage(),
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                ]);
        ...
        $response = DataTables::of($query)
            ...
            ->addColumn('action', function ($row) use ($request, $referenceId) {
                ...
                $deniedButton = $checked ? '' : '<button type="button" class="btn btn-outline-danger denied" value="'.EncryptedId::encode($row->id).'" data-wenk="Menolak data" data-payment="TU" data-type="SPM" data-wenk-color="red"><i class="fa-solid fa-ban"></i></button>';
        ...
        Log::channel('payment_tu')->debug('SP2D TU formJson success', [
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);
