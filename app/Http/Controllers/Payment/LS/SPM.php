...
class SPM extends Controller
{
    ...
    public function json(ActivePositionService $activePosition)
    {
        ...
        if (! $user || ! $user->jabatan) {
            Log::channel('payment_ls')->warning('SPM LS json invalid active position', [
        ...
        Log::channel('payment_ls')->debug('SPM LS json request', [
            'tahun' => $tahun,
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);

        $dataQuery = LS::applySpmJsonScope(
            LS::spmJsonQuery($tahun),
            $jabatanId,
            $unitKerja
        );
        ...
        Log::channel('payment_ls')->debug('SPM LS json success', [
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);
    ...
    public function store(
        ...
        $start = microtime(true);
        Log::channel('payment_ls')->info('SPM LS store request', [
        ...
        try {
            ...
            Log::channel('payment_ls')->info('SPM LS store success', [
                'reference_id' => $idSpp,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls')->error('SPM LS store failed', [
    ...
    {
        $start = microtime(true);
        Log::channel('payment_ls')->debug('SPM LS edit request', [
            'hash' => $request->id,
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);
        ...
        try {
            ...
            $query = LS::rootQuery()
                ...
                ->tap(fn ($q) => LS::joinSpp($q, true))
                ->tap(fn ($q) => LS::joinSp($q, true))
                ->tap(fn ($q) => LS::joinSpm($q, true))
                ->tap(fn ($q) => LS::joinSptjm($q, true))
            ...
            if (! $result) {
                Log::channel('payment_ls')->warning('SPM LS edit not found/forbidden', [
                    'doc_id' => $id,
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                ]);
            ...
            Log::channel('payment_ls')->debug('SPM LS edit success', [
                'doc_id' => $id,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls')->error('SPM LS edit failed', [
                'hash' => $request->id,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
    ...
    public function update(
        ...
        $start = microtime(true);
        Log::channel('payment_ls')->info('SPM LS update request', [
            'selected_spp' => $request->selected_spp,
        ...
        try {
            ...
            Log::channel('payment_ls')->info('SPM LS update success', [
                'spp_id' => $sppId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            $this->cleanupStoredFiles($storedFiles ?? []);
            Log::channel('payment_ls')->error('SPM LS update failed', [
                'spp_id' => $sppId ?? null,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
    ...
    {
        $start = microtime(true);
        Log::channel('payment_ls')->debug('SPM LS formJson request', [
        ...
        $applyUnusedDocumentScope = function ($query) {
            $query->whereNotExists(function ($subQuery) {
                $subQuery->select(DB::raw(1))
                    ...
                    ->where('used_docs.payment_type', 'LS')
        ...
        Log::channel('payment_ls')->debug('SPM LS formJson success', [
            'edited' => $isEdited,
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);
    ...
    public function submit(
        ...
        $start = microtime(true);
        Log::channel('payment_ls')->info('SPM LS submit request', [
            'hash' => $request->id,
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);
        ...
        try {
            ...
            Log::channel('payment_ls')->warning('SPM LS submit invalid parameter', [
                'hash' => $request->id,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        $user = $activePosition->get();
        if (! $user || ! $user->jabatan) {
            Log::channel('payment_ls')->warning('SPM LS submit invalid active position', [
                'doc_id' => $docId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            DB::transaction(function () use ($docId, $userLevel, $userUnit, $documentHistoryService, $start) {
                ...
                if (! $this->canAccessForSubmit($firstDoc, (int) $userLevel, $userUnit)) {
                    Log::channel('payment_ls')->warning('SPM LS submit forbidden document access', [
                        'doc_id' => $docId,
                ...
                Log::channel('payment_ls')->info('SPM LS submit route resolved', [
                    'doc_id' => $docId,
            ...
            Log::channel('payment_ls')->info('SPM LS submit success', [
                'doc_id' => $docId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls')->info('SPM LS submit blocked', [
                'doc_id' => $docId ?? null,
                'reason' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls')->error('SPM LS submit failed', [
                'doc_id' => $docId ?? null,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
