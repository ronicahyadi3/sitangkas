...
class SPP extends Controller
{
    ...
    {
        return view('Payment.LS_Gaji.spp');
    }
        ...
        try {
            ...
            switch ($userJabatan) {
                ...
                case 10:
                    ...
                    break;
                default:
                    Log::channel('payment_ls_gaji')->warning('SPP LS Gaji json unhandled jabatan', [
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                    ]);
                    $dataQuery = LS_GAJI::rootQuery()->whereRaw('1=0');
                    break;
            }
            ...
            $response = DataTables::of($dataQuery)
                ...
                ->addColumn('status', function ($data) use ($userJabatan, $allJabatan, $start) {
                    ...
                    if (is_null($jabatanId)) {
                        Log::channel('payment_ls_gaji')->error('SPP LS Gaji json invalid jabatan mapping', [
                        ...
                        case 7:
                            if ($document->rejected_by_spp || $document->verify_spp) {
                                ...
                                $actions[] = $btn(
                                    ...
                                    'Verifikasi',
                                    'data-payment="LS_GAJI" data-type="SPP"'
                                );
                                $actions[] = $btn(
                                    $enc,
                                    'denied',
            ...
            Log::channel('payment_ls_gaji')->debug('SPP LS Gaji json success', [
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls_gaji')->error('SPP LS Gaji json failed', [
    ...
    public function store(
        ...
        Log::channel('payment_ls_gaji')->info('SPP LS Gaji Store request', [
        ...
        try {
            ...
            Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Store file upload failed', [
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            report($e);
        ...
        try {
            DB::transaction(function () use (
                ...
                Log::channel('payment_ls_gaji')->debug('SPP LS Gaji Store transaction start', [
                    'nomor_spp' => $request->nomor_spp,
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                ]);
            ...
            Log::channel('payment_ls_gaji')->info('SPP LS Gaji Store success', [
                'id_spp' => $sppId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            $this->cleanupStoredFiles($storedFiles);
            Log::channel('payment_ls_gaji')->error('SPP LS Gaji Store failed', [
                'id_spp' => $sppId,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
    ...
    public function edit(
        ...
        Log::channel('payment_ls_gaji')->debug('SPP LS Gaji Edit request', [
            'hash' => $request->id,
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);
        ...
        try {
            ...
            try {
                ...
                Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Edit invalid encrypted ID', [
                    'hash' => $request->id,
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                ]);
            ...
            if (! $data) {
                Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Edit document not found or forbidden', [
                    'id_spp' => $decryptedId,
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                ]);
            ...
            if ($rekening->isEmpty()) {
                Log::channel('payment_ls_gaji')->notice('SPP LS Gaji Edit rekening empty', [
                    'id_spp' => $decryptedId,
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                ]);
            }

            Log::channel('payment_ls_gaji')->debug('SPP LS Gaji Edit success', [
                'id_spp' => $decryptedId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls_gaji')->error('SPP LS Gaji Edit failed', [
                'message' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
    ...
    public function update(
        ...
        $start = microtime(true);
        Log::channel('payment_ls_gaji')->info('SPP LS Gaji Update request', [
            'hash' => $id,
        ...
        try {
            ...
            Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Update invalid ID', [
                'hash' => $id,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            ...
            DB::transaction(function () use (
                ...
                Log::channel('payment_ls_gaji')->debug('SPP LS Gaji Update transaction start', [
                    'id_spp' => $decryptedId,
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                ]);
                ...
                $spj = Document::updateOrCreate(
                    ['reference_id' => $decryptedId, 'src_type' => 'SPJ', 'payment_type' => 'LS_GAJI'],
                    $dataSpj
                );
                ...
                if (count(array_intersect($belanja, [1, 2])) > 0) {
                    ...
                    $bmd = Document::updateOrCreate(
                        ['reference_id' => $decryptedId, 'src_type' => 'BMD', 'payment_type' => 'LS_GAJI'],
                        $dataBmd
                    );
            ...
            Log::channel('payment_ls_gaji')->info('SPP LS Gaji Update success', [
                'id_spp' => $decryptedId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls_gaji')->error('SPP LS Gaji Update failed', [
    ...
    public function submit(Request $request, ActivePositionService $activePosition, DocumentHistoryService $documentHistoryService)
    {
        ...
        Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit request', [
            'hash' => $request->id,
            'duration_ms' => round((microtime(true) - $start) * 1000, 2),
        ]);
        ...
        try {
            ...
            Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Submit Invalid ID', [
                'hash' => $request->id,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            DB::transaction(function () use ($docId, $userLevel, $userUnit, $documentHistoryService, $start) {
                ...
                if ($docs->isEmpty()) {
                    Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Submit document not found', [
                        'doc_id' => $docId,
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                    ]);
                ...
                if (! $this->canAccessForSubmit($mainDoc, $userLevel, $userUnit)) {
                    Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Submit forbidden document access', [
                        'doc_id' => $docId,
                ...
                if ($mainDoc->rejected_by !== null) {
                    Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit already rejected', [
                        'doc_id' => $docId,
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                    ]);
                ...
                Log::channel('payment_ls_gaji')->debug('SPP LS Gaji Submit state', [
                    'doc_id' => $docId,
                ...
                if (! in_array((string) $userLevel, $assignedList, true)) {
                    ...
                    Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit invalid assignment', [
                        'doc_id' => $docId,
                        'assigned_to' => $assignedList,
                        'submit_list' => $submitList,
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                ...
                if ($currentCount >= $maxAllowed) {
                    Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit limit reached', [
                        'doc_id' => $docId,
                        'count' => $currentCount,
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                    ]);
                ...
                Log::channel('payment_ls_gaji')->debug('SPP LS Gaji Submit next assigned', [
                    'doc_id' => $docId,
                ...
                Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit persisted', [
                    'doc_id' => $docId,
                    'submit' => $newSubmit,
                    'assigned_to' => $assignedTo,
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ...
            Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit success', [
                'doc_id' => $docId,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit blocked', [
                'doc_id' => $docId ?? null,
                'reason' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls_gaji')->error('SPP LS Gaji Submit system error', [
                'doc_id' => $docId ?? null,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
    ...
    {
        $start = microtime(true);
        Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit PPTK request', [
            'hash' => $request->id,
        ...
        try {
            ...
            Log::channel('payment_ls_gaji')->debug('SPP LS Gaji  Submit PPTK ID decoded', [
                'doc_id' => $docId,
                'user_to' => $userTo,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
            ...
            Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Submit PPTK invalid parameter', [
                'hash' => $request->id,
                'users_to' => $request->users_to,
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
        ...
        try {
            return DB::transaction(function () use ($docId, $userTo, $userJabatan, $user, $userUnit, $documentHistoryService, $start) {
                ...
                if ($docs->isEmpty()) {
                    Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Submit PPTK document not found', [
                        'doc_id' => $docId,
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                    ]);
                ...
                if (! in_array($userJabatan, [9, 10], true)) {
                    Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Submit PPTK forbidden role', [
                        'doc_id' => $docId,
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                    ]);
                ...
                if (! $this->canAccessForSubmit($firstDoc, $userJabatan, $userUnit)) {
                    Log::channel('payment_ls_gaji')->warning('SPP LS Gaji Submit PPTK forbidden document access', [
                        'doc_id' => $docId,
                ...
                Log::channel('payment_ls_gaji')->debug('SPP LS Gaji Submit PPTK document state', [
                    'doc_id' => $docId,
                ...
                if (! is_null($firstDoc->rejected_by)) {
                    Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit PPTK document rejected', [
                        'doc_id' => $docId,
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                    ]);
                ...
                if (! is_null($firstDoc->submit)) {
                    Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit PPTK already submitted', [
                        'doc_id' => $docId,
                        'submit' => $firstDoc->submit,
                        'duration_ms' => round((microtime(true) - $start) * 1000, 2),
                    ]);
                    ...
                    return response()->json([
                        'status' => 400,
                        'message' => 'Data telah disubmit sebelumnya. Silakan periksa status dokumen.',
                    ], 400);
                }
                ...
                Log::channel('payment_ls_gaji')->info('SPP LS Gaji Submit PPTK success', [
                    'doc_id' => $docId,
                    'assigned_to' => 8,
                    'users_to' => $userTo,
                    'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ...
            Log::channel('payment_ls_gaji')->error('SPP LS Gaji Submit PPTK system failure', [
                'doc_id' => $docId ?? null,
                'error' => $e->getMessage(),
                'duration_ms' => round((microtime(true) - $start) * 1000, 2),
            ]);
