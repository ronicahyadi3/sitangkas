...
class SP2D extends Controller
{
    ...
    {
        return view('Payment.LS_Gaji.sp2d');
    }
    ...
    public function json(ActivePositionService $activePosition)
    {
        ...
        try {
            ...
            if (! $user || ! $user->jabatan) {
                Log::channel('payment_ls_gaji')->warning('SP2D LS Gaji json blocked: invalid active position', [
                        ...
                        case 3:
                            if (! $row->rejected_by) {
                                $actions[] = $btn(
                                    ...
                                    'Menolak Data',
                                    $encSp2d,
                                    'data-payment="LS_GAJI" data-type="SP2D" '
                                );
                            }
                            $actions[] = $btn('history_data', 'ni ni-collection text-info', 'History', $encSpp);
                            break;
            ...
            Log::channel('payment_ls_gaji')->debug('SP2D LS Gaji json success', [
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);

            return $response;
        } catch (\Throwable $e) {
            Log::channel('payment_ls_gaji')->error('SP2D LS Gaji json failed', [
                'error' => $e->getMessage(),
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
            report($e);
    ...
    public function store(
        ...
        $user = $activePosition->get();
        if (! $user || ! $user->jabatan) {
            Log::channel('payment_ls_gaji')->warning('SP2D LS Gaji store blocked: invalid active position', [
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
        ...
        Log::channel('payment_ls_gaji')->info('SP2D LS Gaji store request', [
        ...
        try {
            ...
            Log::channel('payment_ls_gaji')->info('SP2D LS Gaji store success', [
                'reference_id' => $referenceId,
                'user_to' => $userTo,
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
            ...
            Log::channel('payment_ls_gaji')->error('SP2D LS Gaji store failed', [
    ...
    public function edit(Request $request, ActivePositionService $activePosition)
    {
        ...
        Log::channel('payment_ls_gaji')->debug('SP2D LS Gaji edit request', [
            'hash' => $request->id,
            'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
        ]);
        try {
            ...
            $user = $activePosition->get();
            if (! $user || ! $user->jabatan) {
                Log::channel('payment_ls_gaji')->warning('SP2D LS Gaji edit blocked: invalid active position', [
                    'hash' => $request->id,
                    'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
                ]);
            ...
            if (! $result) {
                Log::channel('payment_ls_gaji')->warning('SP2D LS Gaji edit not found', [
                    'doc_id' => $id,
                    'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
                ]);
            ...
            Log::channel('payment_ls_gaji')->debug('SP2D LS Gaji edit success', [
                'doc_id' => $id,
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
            ...
            Log::channel('payment_ls_gaji')->error('SP2D LS Gaji edit failed', [
                'hash' => $request->id,
                'error' => $e->getMessage(),
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
    ...
    public function update(
        ...
        $user = $activePosition->get();
        if (! $user || ! $user->jabatan) {
            Log::channel('payment_ls_gaji')->warning('SP2D LS Gaji update blocked: invalid active position', [
        ...
        Log::channel('payment_ls_gaji')->info('SP2D LS Gaji update request', [
            'hash' => $request->route('id'),
            'sp2d_id' => $sp2dId,
            'reference_id' => $referenceId,
        ...
        try {
            ...
            Log::channel('payment_ls_gaji')->info('SP2D LS Gaji update success', [
                'sp2d_id' => $sp2dId,
                'reference_id' => $referenceId,
                'spm_unit_kerja_id' => $unitKerjaId,
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ...
            Log::channel('payment_ls_gaji')->error('SP2D LS Gaji update failed', [
                'sp2d_id' => $sp2dId ?? null,
                'error' => $e->getMessage(),
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
    ...
    public function formJson(Request $request, ActivePositionService $activePosition)
    {
        ...
        $user = $activePosition->get();
        if (! $user || ! $user->jabatan) {
            Log::channel('payment_ls_gaji')->warning('SP2D LS Gaji formJson blocked: invalid active position', [
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
        ...
        try {
            ...
            Log::channel('payment_ls_gaji')->warning('SP2D LS Gaji formJson invalid data hash', [
                'data' => $request->data,
                'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
        ...
        Log::channel('payment_ls_gaji')->debug('SP2D LS Gaji formJson request', [
        ...
        $dataQuery = LS_GAJI::rootQueryAlias('spm')
            ->where('spm.src_type', 'SPM')
            ->where('spm.payment_type', 'LS_GAJI')
            ...
            ->leftJoin('document as spp', function ($join) {
                $join->on('spp.id', '=', 'spm.reference_id')
                    ...
                    ->where('spp.payment_type', 'LS_GAJI')
            ...
            ->where(function ($q) use ($isEdited, $referenceId) {
                $q->whereNotExists(function ($sub) {
                    $sub->select(DB::raw(1))
                        ...
                        ->where('sp2d.payment_type', 'LS_GAJI')
        ...
        Log::channel('payment_ls_gaji')->debug('SP2D LS Gaji formJson success', [
            'edited' => $isEdited,
            'duration_ms' => (int) round((microtime(true) - $startedAt) * 1000),
        ]);
