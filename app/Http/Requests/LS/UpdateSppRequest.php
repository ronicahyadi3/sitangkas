...
class UpdateSppRequest extends FormRequest
{
    ...
    public function authorize(
        ...
        if (
            ! $yearAccess->canWrite($position, $selectedYear)
            || $selectedYear > $yearAccess->currentYear()
        ) {
            return false;
        }

        $documentId = EncryptedId::tryDecode($this->route('id'));
        ...
        $spp = Document::query()
            ...
            ->whereKey($documentId)
            ->where('id_unit_kerja', $position->unitKerja->getKey())
    ...
    public function rules(): array
    {
        return [
            ...
            'file_spp' => ['bail', 'nullable', 'file', 'mimes:pdf', 'max:'.self::MAX_PDF_SIZE_KILOBYTES],
            'file_spj' => ['bail', 'nullable', 'file', 'mimes:pdf', 'max:'.self::MAX_PDF_SIZE_KILOBYTES],
            'file_billing' => ['bail', 'nullable', 'file', 'mimes:pdf', 'max:'.self::MAX_PDF_SIZE_KILOBYTES],
            'file_bmd' => [
                'bail',
                Rule::requiredIf(fn (): bool => $this->requiresBmdFile() && ! $this->hasExistingBmd()),
                'nullable',
                'file',
                'mimes:pdf',
    ...
    public function after(YearAccessService $yearAccess): array
    {
        return [
            function (Validator $validator) use ($yearAccess): void {
                ...
                if (
                    ! $position instanceof UserPosition
                    || ! $position->unitKerja
                    || ! $spp instanceof Document
                ) {
                    $validator->errors()->add('rekening', 'Dokumen SPP atau posisi aktif tidak valid.');
    ...
    protected function prepareForValidation(): void
    {
        ...
        foreach (['file_spp', 'file_spj', 'file_billing', 'file_bmd'] as $field) {
    ...
    protected function failedAuthorization(): never
    {
        throw new AuthorizationException(
            'Anda tidak berwenang mengubah SPP ini atau dokumen sudah masuk proses persetujuan.',
        );
    }

    private function paymentType(): string
    {
        $controllerAction = $this->route()?->getActionName() ?? '';

        return str_contains($controllerAction, '\\Payment\\LS_Gaji\\')
            ? 'LS_GAJI'
            : 'LS';
    }

    private function hasExistingBmd(): bool
    {
        if (! $this->spp instanceof Document) {
            return false;
        }
        ...
        return Document::query()
            ...
            ->where('reference_id', $this->spp->getKey())
            ->exists();
    }
