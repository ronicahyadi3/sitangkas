            ...
            <div class="card-body py-3">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        ...
                        <div class="text-sm text-secondary">Dokumen GU SKPD SPM hanya dapat dilihat melalui detail dokumen dan riwayat perubahan.</div>
                    </div>
...
@section('additionals')
    ...
    @unless ($isAuditor)
        ...
        @include('components.confirmations.submit', [
            'url' => route('gu-skpd.spm.submit'),
        ])
        @include('components.form.formCrud', [
            'urlStore' => route('gu-skpd.spm.store'),
            'urlUpdate' => route('gu-skpd.spm.update', ':id'),
            'urlEdit' => route('gu-skpd.spm.edit', ':id'),
            'modalId' => 'add',
            'submitId' => 'submit',
        ])
    @endunless
    ...
    @unless ($isAuditor)
        @include('components.informations.Table', [
            'url' => route('gu-skpd.spm.lpj.json'),
            'nameTable' => 'formTable',
            'jabatanAccessAdd' => [],
            'data' => [
                'edited' => 'Edited',
    ...
    <script>
        ...
        function buildFormData() {
            ...
            formData.append('selected_lpj', $('input[name="selected_lpj"]:checked').val() || '');
        ...
        function StoreData() {
            if (!$('input[name="selected_lpj"]:checked').val()) {
                notification({
                    status: 400,
                    message: 'Pilih dokumen LPJ terlebih dahulu.'
                });
                return;
            }
            FormCrud.store();
