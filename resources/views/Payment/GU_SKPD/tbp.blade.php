            ...
            <div class="card-body py-3">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        ...
                        <div class="text-sm text-secondary">Dokumen GU SKPD TBP hanya dapat dilihat melalui detail dokumen dan riwayat perubahan.</div>
                    </div>
                    ...
                    <div class="modal-body p-0">
                        <div class="card card-plain">
                            ...
                            <div class="card-body">
                                ...
                                <label>Dokumen SPJ</label>
                                <div class="mb-3">
                                    <div class="input-group">
                                        <span class="input-group-text pe-3"><i class="fa-solid fa-file-pdf fa-lg"></i></span>
                                        <input type="text" id="nomor_spj" class="form-control"
                                            readonly>
                                        <label class="btn btn-primary mb-0" for="file_spj">Upload</label>
                                        <input type="file" id="file_spj" accept="application/pdf" class="d-none" />
                                    </div>
                                    <small class="form-text">Nama File : <span id="name_spj" class="font-weight-bold">Belum ada
                                            file yang dipilih</span></small>
                                </div>

                                <label>Dokumen Billing</label>
                                <div class="mb-3">
                                    <div class="input-group">
                                        <span class="input-group-text pe-3"><i class="fa-solid fa-file-pdf fa-lg"></i></span>
                                        <input type="text" id="nomor_billing" class="form-control"
                                            readonly>
                                        <label class="btn btn-primary mb-0" for="file_billing">Upload</label>
                                        <input type="file" id="file_billing" accept="application/pdf" class="d-none" />
                                    </div>
                                    <small class="form-text">Nama File : <span id="name_billing" class="font-weight-bold">Belum
                                            ada file yang dipilih</span></small>
                                </div>
...
@section('additionals')
    ...
    @unless ($isAuditor)
        ...
        @include('components.confirmations.submit', [
            'url' => route('gu-skpd.tbp.submit'),
        ])

        @include('components.form.formCrud', [
            'urlStore' => route('gu-skpd.tbp.store'),
            'urlUpdate' => route('gu-skpd.tbp.update', ':id'),
            'urlEdit' => route('gu-skpd.tbp.edit', ':id'),
            'modalId' => 'add',
            'submitId' => 'submit',
        ])
    @endunless
    ...
    @include('components.informations.Table', [
        ...
        'title' => 'Dokumen TBP',
        'url' => route('gu-skpd.tbp.json'),
        'nameTable' => 'mainTable',
        'jabatanAccessAdd' => [9, 10, 1],
        'columns' => [
            [
                'data' => 'DT_RowIndex',
                'class' => 'text-center',
    ...
    @unless ($isAuditor)
        @include('components.informations.Table', [
            'url' => route('gu-skpd.tbp.npd.json'),
            'nameTable' => 'formTable',
            'jabatanAccessAdd' => [],
            'data' => [
                'edited' => 'Edited',
        ...
        <script>
            function bindSimpleFileName(inputId, labelId) {
                const input = document.getElementById(inputId);
                input?.addEventListener('change', function() {
                    const f = this.files?.[0];
                    document.getElementById(labelId).textContent = f ? f.name : 'Belum ada file yang dipilih';
                });
            }

            bindSimpleFileName('file_spj', 'name_spj');
            bindSimpleFileName('file_billing', 'name_billing');
