            ...
            <div class="card-body py-3">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        ...
                        <div class="text-sm text-secondary">Dokumen SPP UP hanya dapat dilihat melalui detail dokumen dan riwayat perubahan.</div>
                    </div>
                    ...
                    <div class="modal-body p-0">
                        <div class="card card-plain">
                            <div class="card-header pb-0 d-flex justify-content-between">
                                <div>
                                    <h3 class="font-weight-bolder text-info text-gradient">Unggah Dokumen SPP UP</h3>
                                    <p class="mb-0">Isi nomor SPP, file PDF, uraian, dan nominal.</p>
                                </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3 input-group">
                                    ...
                                    <textarea id="uraian" class="form-control" rows="3" placeholder="Uraian pencairan"
                                        onfocus="this.placeholder=''"></textarea>
                                </div>

                                <label>Nominal</label>
                                <div class="mb-3 input-group">
                                    <span class="input-group-text pe-3"><i class="fa-solid fa-rupiah-sign"></i></span>
...
@section('additionals')
    ...
    @unless ($isAuditor)
        ...
        @include('components.confirmations.submit', [
            'url' => route('up.spp.submit'),
        ])

        @include('components.form.formCrud', [
            'urlStore' => route('up.spp.store'),
            'urlUpdate' => route('up.spp.update', ':id'),
            'urlEdit' => route('up.spp.edit', ':id'),
            'modalId' => 'add',
            'submitId' => 'submit',
        ])
    @endunless
    ...
    @include('components.informations.Table', [
        'subtitle' => 'Pencairan UP',
        'title' => 'Dokumen SPP',
        'url' => route('up.spp.json'),
        'nameTable' => 'mainTable',
        'jabatanAccessAdd' => [9],
        'columns' => [
            [
            ...
            ['data' => 'nomor_spp', 'class' => 'text-start text-wrap', 'title' => 'Nomor SPP'],
        ...
        @accessJabatan([9])
            <script>
                ...
                function buildFormData() {
                    ...
                    formData.append('nominal', Number(AutoNumeric.getNumber('#nominal') || 0).toFixed(2));

                    const fileSpp = $('#file_spp')[0].files[0];
                    if (fileSpp) {
                        formData.append('file_spp', fileSpp);
                    }
                ...
                function formEdit(res, id) {
                    ...
                    $('#nomor_spp').val(res.data.nomor || '');
                    $('#uraian').val(res.data.uraian || '');
                    nominal.set(res.data.nominal || 0);
                    $('#file_spp').val('');
