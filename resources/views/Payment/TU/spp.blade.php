            ...
            <div class="card-body py-3">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        ...
                        <div class="text-sm text-secondary">Dokumen SPP TU hanya dapat dilihat melalui detail dokumen dan riwayat perubahan.</div>
                    </div>
                    ...
                    <div class="modal-body p-0">
                        <div class="card card-plain">
                            <div class="card-header pb-0 d-flex justify-content-between">
                                <div>
                                    <h3 class="font-weight-bolder text-info text-gradient">Unggah Dokumen SPP TU</h3>
                                    <p class="mb-0">Pilih Pengajuan, isi nomor SPP, upload PDF, dan isi rekening anggaran</p>
                                </div>
...
@section('additionals')
    ...
    @unless ($isAuditor)
        ...
        @include('components.confirmations.submit', [
            'url' => route('tu.spp.submit'),
        ])
        @include('components.confirmations.submitPPTK', [
            'url' => route('tu.spp.submit.pptk'),
        ])
        ...
        @include('components.form.formCrud', [
            'urlStore' => route('tu.spp.store'),
            'urlUpdate' => route('tu.spp.update', ':id'),
            'urlEdit' => route('tu.spp.edit', ':id'),
            'modalId' => 'add',
            'submitId' => 'submit',
        ])
    @endunless
    ...
    @include('components.informations.Table', [
        ...
        'title' => 'Dokumen SPP',
        'url' => route('tu.spp.json'),
        'nameTable' => 'mainTable',
        'jabatanAccessAdd' => [1, 9, 10],
        'columns' => [
            [
                'data' => 'DT_RowIndex',
                'class' => 'text-center',
    ...
    @unless ($isAuditor)
        @include('components.informations.Table', [
            'url' => route('tu.spp.pengajuan.json'),
            'nameTable' => 'formTable',
            'jabatanAccessAdd' => [],
            'data' => [
                'edited' => 'Edited',
        ...
        @accessJabatan([1, 9, 10])
            <script>
                function buildFormData() {
                    const formData = new FormData();
                    formData.append('nomor_spp', $('#nomor_spp').val());
                    formData.append('uraian', $('#uraian').val());
                    formData.append('selected_pengajuan', $('input[name="selected_pengajuan"]:checked').val());
                ...
                function StoreData() {
                    if (!$('input[name="selected_pengajuan"]:checked').val()) {
                        notification({
                            status: 400,
                            message: 'Pilih dokumen pengajuan terlebih dahulu.'
                        });
                        return;
                    }
