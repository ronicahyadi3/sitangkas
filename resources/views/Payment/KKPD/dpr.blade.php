            ...
            <div class="card-body py-3">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        ...
                        <div class="text-sm text-secondary">Dokumen DPR KKPD hanya dapat dilihat melalui detail dokumen dan riwayat perubahan.</div>
                    </div>
                    ...
                    <div class="modal-body p-0">
                        <div class="card card-plain">
                            <div class="card-header pb-0 d-flex justify-content-between">
                                <div>
                                    <h3 class="font-weight-bolder text-info text-gradient">Unggah Dokumen DPR KKPD</h3>
                                    <p class="mb-0">Lengkapi dokumen DPR, NPD, dan DPT</p>
                                </div>
...
@section('additionals')
    ...
    @unless ($isAuditor)
        ...
        @include('components.confirmations.submit', [
            'url' => route('kkpd.dpr.submit'),
        ])

        @include('components.form.formCrud', [
            'urlStore' => route('kkpd.dpr.store'),
            'urlUpdate' => route('kkpd.dpr.update', ':id'),
            'urlEdit' => route('kkpd.dpr.edit', ':id'),
            'modalId' => 'add',
            'submitId' => 'submit',
        ])
    @endunless
    ...
    @include('components.informations.Table', [
        ...
        'columns' => [
            ...
            ['data' => 'nomor_dpt', 'class' => 'text-start text-wrap', 'render' => 'renderNomor', 'title' => 'Nomor DPT'],
            [
                'data' => 'status_button',
                'class' => 'text-center',
                'searchable' => false,
        ...
        @accessJabatan([1, 8])
            <script>
                function buildFormData() {
                    ...
                    formData.append('nomor_dpr', $('#nomor_dpr').val());
                    formData.append('nomor_dpt', $('#nomor_dpt').val());

                    const fileDpr = $('#file_dpr')[0].files[0];
                    const fileNpd = $('#file_npd')[0].files[0];
                    const fileDpt = $('#file_dpt')[0].files[0];

                    if (fileDpr) {
                        formData.append('file_dpr', fileDpr);
                    }

                    if (fileNpd) {
                        formData.append('file_npd', fileNpd);
                    }

                    if (fileDpt) {
                        formData.append('file_dpt', fileDpt);
                    }
                ...
                function resetFormAdd() {
                    ...
                    $('#file_npd').val('');
                    $('#file_dpt').val('');
                    $('#name_dpr').text('Belum ada file yang dipilih');
                    $('#name_npd').text('Belum ada file yang dipilih');
                    $('#name_dpt').text('Belum ada file yang dipilih');
                }
                ...
                function formEdit(res, id) {
                    ...
                    $('#nomor_dpr').val(res.data.dpr?.nomor || '');
                    $('#nomor_npd').val('');
                    $('#nomor_dpt').val(res.data.dpt?.nomor || '');
                    $('#file_dpr').val('');
                    $('#file_npd').val('');
                    $('#file_dpt').val('');
                    $('#name_dpr').text('Kosongkan jika file tidak diganti');
                    $('#name_npd').text('Kosongkan jika file tidak diganti');
                    $('#name_dpt').text('Kosongkan jika file tidak diganti');
                }
