            ...
            <div class="card-body py-3">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        ...
                        <div class="text-sm text-secondary">Paket LPJ KKPD dan dokumen turunannya dapat dilihat tanpa fitur kelola, submit, atau tolak.</div>
                    </div>
                            ...
                            <div class="card-header pb-0 d-flex justify-content-between">
                                <div>
                                    <h3 class="font-weight-bolder text-info text-gradient" id="managePackageModalLabel">
                                        {{ $canManagePackage ? 'Kelola Dokumen LPJ KKPD' : 'Dokumen LPJ KKPD' }}
                                    </h3>
                                    <p class="mb-0">
...
@section('additionals')
    ...
    @if ($canViewPackage)
        @include('components.informations.Table', [
            'url' => route('kkpd.lpj.documents.json'),
    ...
    <script>
        ...
        async function saveChildDocument() {
            ...
            const url = currentEditingDocumentId ?
                @json(route('kkpd.lpj.update', ':id')).replace(':id', currentEditingDocumentId) :
                @json(route('kkpd.lpj.store'));
        ...
        async function loadChildDocumentForEdit(id) {
            $.ajax({
                type: 'GET',
                url: @json(route('kkpd.lpj.edit', ':id')).replace(':id', id),
                dataType: 'json',
                success(response) {
        ...
        async function deleteChildDocument(id) {
            ...
            $.ajax({
                type: 'POST',
                url: @json(route('kkpd.lpj.delete', ':id')).replace(':id', id),
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success(response) {
