...
Route::middleware(['auth', 'account.accessible', 'single.device.session'])->group(function (): void {
    ...
    Route::middleware(['has.position', 'mfa.verified', 'active.position', 'password.fresh'])->group(function (): void {
        ...
        Route::prefix('document')->name('document.')->group(function (): void {
            ...
            Route::get('/ls/spj/{document}/content', [LsSpjDocumentDeliveryController::class, 'content'])
                ->where('document', '[A-Za-z0-9_-]+')
                ->name('ls.spj.content');
            ...
            Route::get('/ls/billing/{document}/content', [LsBillingDocumentDeliveryController::class, 'content'])
                ->where('document', '[A-Za-z0-9_-]+')
                ->name('ls.billing.content');
            Route::get('/ls/billing/{document}/download', [LsBillingDocumentDeliveryController::class, 'download'])
                ->where('document', '[A-Za-z0-9_-]+')
                ->name('ls.billing.download');
