...
| Integrasi BSrE/eSign Client 2.2.0, TTE asynchronous/queue, enum/status/transition, verifikasi PDF, atau status sertifikat | `../08-esign/README.md` | `ESIGN_V2_CONTRACT_AND_BACKEND.md`, lalu `ESIGN_V2_IMPLEMENTATION_PLAN.md` |
| Signer, Admin Super/multi-position/acting pada TTE, `SELF_SIGN`, assignment SP2D, penempatan QR/footer, urutan signer, reject/revisi, `needs_review`, pembatalan preview tanpa record, retry, atau policy view/download dokumen | `../08-esign/ESIGN_AUTHORIZATION_AND_WORKFLOW_MATRIX.md` | `../08-esign/ESIGN_V2_CONTRACT_AND_BACKEND.md`, lalu controller payment/`document_process` legacy sebagai bukti read-only |
...
| Modal TTE/validasi, Svelte, Vite, Bootstrap 5, Argon, main CSS, PDF viewer, atau penempatan tanda tangan | `../08-esign/README.md` | `ESIGN_V2_FRONTEND_MODAL.md`, lalu bagian frontend pada `ESIGN_V2_IMPLEMENTATION_PLAN.md` |
...
| `app/Services/Esign/*`, `app/Contracts/Esign/*`, `app/Data/Esign/*`, `app/Actions/Esign/*`, `app/Jobs/Esign/*` | `../08-esign/README.md`, `../08-esign/ESIGN_V2_CONTRACT_AND_BACKEND.md`, dan `../08-esign/ESIGN_V2_IMPLEMENTATION_PLAN.md` |
...
| Controller, request, policy, route, atau migration untuk signing session/TTE/verifikasi | `../08-esign/README.md`, `../08-esign/ESIGN_V2_CONTRACT_AND_BACKEND.md`, `../08-esign/ESIGN_V2_IMPLEMENTATION_PLAN.md`; tambah docs auth/posisi/tahun sesuai scope |
| `resources/js/esign/*`, komponen Svelte TTE/validasi, PDF viewer, atau adapter Blade untuk eSign | `../08-esign/README.md` dan `../08-esign/ESIGN_V2_FRONTEND_MODAL.md` |
| Blade/bundle legacy yang memuat `signModal`, `ConfirmSign`, `ModalViewSignature`, atau `pdfViewerModal` | `../08-esign/ESIGN_V2_FRONTEND_MODAL.md`, lalu `../99-legacy/OLD_PROJECT_REFERENCE.md` hanya untuk membandingkan perilaku |
...
| Migration/model/service untuk `document_artifacts`, attempt events, legacy sign links, public verification, atau compatibility writer | `../08-esign/ESIGN_DOCUMENT_LIFECYCLE_AND_REPORTING_COMPATIBILITY.md`, lalu `../08-esign/ESIGN_V2_CONTRACT_AND_BACKEND.md` dan `../06-migrations/FRESH_INSTALL_READINESS.md` |
